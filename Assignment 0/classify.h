...
class alignas(64) Counter { // Aligned allocation per counter. Is that enough?
			    // Keeps per-thread subcount.
public:
   Counter(unsigned int num=MAXTHREADS) {
      _numcount = num;
      _counts = new unsigned int[num*PAD1];
      assert(_counts != NULL);
      zero();
   }
   ...
   unsigned int get(unsigned int id) const { // return subcounter value for specific thread
      assert(id < _numcount);
      return _counts[id*PAD1];
   }

   void inspect() {
      std::cout << "Subcounts -- ";
      for(int i=0; i<_numcount; i++)
	 std::cout << i << ":" << _counts[i*PAD1] << " ";
      std::cout << "\n";
   }
