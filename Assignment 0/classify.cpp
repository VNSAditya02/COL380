...
{ // Classify each item in D into intervals (given by R). Finally, produce in D2 data sorted by interval
   ...
   Counter counts[R.num()*PAD2]; // I need on counter per interval. Each counter can keep pre-thread subcount.
   #pragma omp parallel num_threads(numt)
   {
      ...
      for(int i=tid; i<D.ndata; i+=numt) { // Threads together share-loop through all of Data
         ...
         counts[v*PAD2].increase(tid); // Found one key in interval v
      }
   }

   // Accumulate all sub-counts (in each interval;'s counter) into rangecount
   unsigned int *rangecount = new unsigned int[R.num()];
   int y = ceil((double)R.num()/numt);
   #pragma omp parallel num_threads(numt)
   {
      int tid = omp_get_thread_num();
      int end = std::min(tid*y + y, R.num());
      for(int r=tid*y; r<end; r+=1) { // For all intervals
         rangecount[r] = 0;
         for(int t=0; t<numt; t++) // For all threads
            rangecount[r] += counts[r*PAD2].get(t);
   ...
   #pragma omp parallel num_threads(numt)
   {
      int tid = omp_get_thread_num();
      int end = std::min(tid*y + y, R.num());
      for(int r=tid*y; r<end; r+=1) { // Thread together share-loop through the intervals 
         int rcount = 0;
